                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        <div class="box-tools pull-right">
                            ...
                            <?php } ?> 
                            <!-- na if statement -->
                            <?php if($view_add_med == 7 || $view_add_med == 0){ if ($this->rbac->hasPrivilege('pathology_test', 'can_view')) { ?>   
...
<div class="modal fade" id="addBarcodeModal" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content modal-media-content">
            <form action="#" method="POST" id="form-report_barcode">
            <div class="modal-header modal-media-header">
                <button type="button" class="close" data-toggle="tooltip" title="<?php echo $this->lang->line('clase'); ?>" data-dismiss="modal">&times;</button>
                <div class="modalicon"> 
                    <div id='collection_modal_header'>
            ...
            <div class="scroll-area">
                <div class="modal-body-barcode noPrint pb0 ptt10" id="mydiv">
      ...
      <div class="modal-footer">
      <button type="submit"  data-loading-text="<?php echo $this->lang->line('processing'); ?>" class="btn btn-info pull-right" ><i class="fa fa-check-circle"></i> Print</button>
      <!-- <button onclick="window.print()" class="noPrint">Print Me
</button> -->
<!-- <input type="button" value="Print Div" onclick="('#mydiv')".print();> -->
...
$(document).on('click','.add_barcode',function(){
       ...
       alert("add barcode");
       alert(id);
       var modal_view=$('#addBarcodeModal');
       var $this = $(this);  
    //    console.log($this);

       $.ajax({
            url: base_url+'admin/pathology/getPathologyBarcodeDetail',
            type: "POST",
            data: {'id': id},
            dataType: 'json',
               beforeSend: function() {
            ...
            success: function (data) {       
            $('#addBarcodeModal .modal-body-barcode').html(data.page);
            $('#addBarcodeModal .filestyle').dropify();
...
$(document).on('click','.print_pathology_report',function(){
 ...
 $.ajax({
    ...
    //na for varcode
    $(document).on('submit','#form-report_barcode',function(e){
        alert("print");
        e.preventDefault();
        $('#viewDetailReportModal').modal('hide');
        $('.box-primary').hide();
...
<script type="text/javascript">
   ...
   function popup(data)
    ...
    $(document).ready(function (e) {
        $('#viewDetailReportModal,#addPaymentModal,#collectionModal,#addReportModal,#addBarcodeModal').modal({
        backdrop: 'static',
        keyboard: false,
        show:false
        });
