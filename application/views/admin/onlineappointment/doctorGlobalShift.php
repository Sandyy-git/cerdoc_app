                        ...
                        <div class="table-responsive mailbox-messages">
                            <table class="table table-hover table-striped table-bordered example">
                                ...
                                <!-- DOCTOR LOGIN START -->
                                    ...
                                    <!-- CLINIC LOGIN START -->
                                    <?php if(!empty($clinic_shift)){ 
                                      
                                        $keys = array_keys($clinic_shift);
                                        
                                        // for($i = 0; $i < count($clinic_shift); $i++) {
                                           foreach ($clinic_shift as $shift_key => $shift_value) {
                                    //         echo "<pre>";
                                    //   print_r($shift_value);
                                    ...
                                    <tr>
                                        <td class="mailbox-name">
                                            <a href="#" data-toggle="popover" class="detail_popover"><?php echo $shift_value[0]['first_name']." ".$shift_value[0]["surname"]; ?> (<?php echo $shift_value[0]["employee_id"]; ?>)</a>
                                        </td>
                                        <?php foreach ($global_shift as $gkey => $gvalue) { 
                                            ...
                                            <td>
                                               ...
                                               <input type="checkbox" 
                                               <?php if ($this->rbac->hasPrivilege('online_appointment_doctor_shift', 'can_edit')) { ?>
                                                    onclick="changeShift(<?php echo $shift_value[0]['id']; ?>,<?php echo $gvalue['id']; ?>,this)" 
                                               <?php }else{
                                                   echo " disabled";
                                               } ?>
                                               id="global_shift_<?php echo $gvalue['id']; ?>" name="global_shift[]" value="<?php echo $gvalue['id']; ?>" data-id = <?= $shift_value[0]['id'].$gvalue['id']; ?>
                                               <?php if(in_array($gvalue['id'], $doctor_shift)){echo "checked=checked";} ?>
                                               />
                                                <span class="hide" id="checkbox_print_<?= $shift_value[0]['id'].$gvalue['id']; ?>">
                                                    <?php 
                                                        if(in_array($gvalue['id'], $doctor_shift)){
                                                            echo $this->lang->line("yes");
                                                        }else{
