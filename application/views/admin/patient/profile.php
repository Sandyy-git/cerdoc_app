...
//REF WINDOW CODE STARTS
//Collecting select2 input value
    // $(document).on('change','.select2-search__field',function(){
    //     document.getElementById('health_institutions1').innerHTML="";
    //     alert($(this).val());   //to collect select2 inut value
    //     var seld = $('.medicine_name').select2();
    //     console.log(seld);
    //     getMedicineTwo(seld,$(this).val(),0);

    // });

    function getMedicineTwo(med_cat_obj,val,medicine_id){
    //  alert(medicine_id); 
    //  alert(val);
    //  alert(med_cat_obj);    
    //   var medicine_colomn=med_cat_obj.closest('tr').find('.medicine_name');
    //   console.log(medicine_colomn);
        // medicine_colomn.html("");    
        $.ajax({
            url: '<?php echo base_url(); ?>admin/pharmacy/get_medicine_searchBy_name',
            type: "POST",
            data: {medicine_category_id: val},
            dataType: 'json',
            //   beforeSend: function() {
...
$.each(res, function (i, obj) {
    ...
    myselect2.append($('<option ></option>').val(obj.medicine_name).html(obj.medicine_name+" "+obj.generic_name+" "+obj.medicine_composition));
});
    ...
    $(document).on('click','.add-record',function(){
         ...
         var st_row = "" ;
            if(rowCount==0){
              ...
              afbf_row =" <label><?php echo $this->lang->line("af/bf"); ?></label>";
