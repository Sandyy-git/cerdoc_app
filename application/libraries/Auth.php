...
class Auth
{
    ...
    public function app_routine()
    {
        ...
        if (($lst_update_month < date("n", strtotime("first day of previous month")) and $lst_update_year == date('Y')) or ($lst_update_month > date('m') and $lst_update_year < date('Y')) or ($update_session >= date('d') and $lst_update_month < date('m') and $lst_update_year == date('Y')) or ($update_session < date('d') and $lst_update_month == date("n", strtotime("first day of previous month")) and $lst_update_year == date('Y'))) {
            ...
            $ehealthflex_market_purchase_code = $this->CI->config->item('ehealthflex_market_purchase_code');
            $ehealthflex_market_username      = $this->CI->config->item('ehealthflex_market_username');
            ...
            if (!file_exists($file_license)) {
                $ehealthflex_market_purchase_code = "tempered";
                $ehealthflex_market_username      = 'tempered';
            }
            $post = [
                'ip'          => $ip,
                'site_url'    => base_url(),
                ...
                'empc'        => $ehealthflex_market_purchase_code,
                'em_username' => $ehealthflex_market_username,
            ];
            $ch = curl_init();
    ...
    public function app_update()
    {
        ...
        $ehealthflex_market_purchase_code = $this->CI->input->post('ehealthflex_market_purchase_code');
        ...
        $data = array(
            'email'         => $email,
            'purchase_code' => $ehealthflex_market_purchase_code,
            'base_url'      => base_url(),
        );
    ...
    public function andapp_update()
    {
        ...
        $ehealthflex_market_purchase_code = $this->CI->input->post('app-ehealthflex_market_purchase_code');
        $shlk                        = $this->CI->config->item('SHLK');
