...
/////
        // $adminid = $this->_CI->notificationsetting_model->getstaffidByID(1);  //29,103
        $supperadminid = $this->_CI->notificationsetting_model->getstaffidByID(7);  //1,3
//////
        //na
        $staffid  = $this->_CI->customlib->getStaffID();
        ...
        //na
        // if($staffid != 7 || $staffid != 1){
        //     $role                        = $this->_CI->customlib->getStaffRole();
    ...
    //new
        $parent_child = $this->_CI->notificationsetting_model->getstaffidtosendSysNotification($event_variables['doctor_id']);
        if(!empty($parent_child)){
            foreach($parent_child as $parent_child_value){
                $notification_data[] = array(
                    'notification_title' => $event_data['subject'],
                    'notification_desc'                             => $staff_message,
                    'role_id'                                       => $parent_child_value['role_id'],
                    'receiver_id'                                   => $parent_child_value['id'],
                    'notification_type'                             => $event_data['notification_type'],
                    'date'                                          => date('Y-m-d H:i:s'),
                    'is_active'                                     => 'yes',
                );
    ...
    //new end
        ...
        if($event=='notification_appointment_created' && $event_data['is_active']==1){
            ...
            foreach ($notification_array['visible_module'] as $key => $visible_value) {
                $staff_id = $visible_value;
                $role_data = $this->_CI->role_model->getRoleFromStaffIf($staff_id);
