...
class Admin extends Admin_Controller
{
    ...
    {
        // var_dump($this->rbac->hasPrivilege()); die;
        $data['title']      = 'Search';
        $userdata           = $this->customlib->getUserData();
    ...
    public function dashboard()
    {
       ...
       $parentid= $staffid;
        ...
        while ($start_month <= $end) {
            ...
            $return        = $this->transaction_model->get_monthTransaction($where_date,null);
        ...
        $data['opd_income']        = $this->transaction_model->get_monthTransaction($search_opd_income,$parentid);
        $search_ipd_income                    = array('payment_date >=' => date('Y-m-01'), 'payment_date <=' => date("Y-m-t"),'ipd_id !='=>null);
        $data['ipd_income']        = $this->transaction_model->get_monthTransaction($search_ipd_income,$parentid);
        $search_pharmacy_income                    = array('payment_date >=' => date('Y-m-01'), 'payment_date <=' => date("Y-m-t"),'pharmacy_bill_basic_id !='=>null);
        $data['pharmacy_income']   = $this->transaction_model->get_monthTransaction($search_pharmacy_income,$parentid);
        $search_pathology_income                    = array('payment_date >=' => date('Y-m-01'), 'payment_date <=' => date("Y-m-t"),'pathology_billing_id !='=>null);
        $data['pathology_income']  = $this->transaction_model->get_monthTransaction($search_pathology_income,$parentid);
        $search_radiology_income                    = array('payment_date >=' => date('Y-m-01'), 'payment_date <=' => date("Y-m-t"),'radiology_billing_id !='=>null);
        $data['radiology_income']  = $this->transaction_model->get_monthTransaction($search_radiology_income,$parentid);
        ...
        $label  = array($this->lang->line('opd'), $this->lang->line('ipd'), $this->lang->line('pharmacy'), $this->lang->line('pathology'), $this->lang->line('radiology'), $this->lang->line('income'));
        $module = array('opd', 'ipd', 'pharmacy', 'pathology', 'radiology', 'income');
    ...
    public function updateandappCode()
    {
        ...
        $this->form_validation->set_rules('app-ehealthflex_market_purchase_code', $this->lang->line('purchase_code'), 'required|trim|xss_clean');

        if ($this->form_validation->run() == false) {
            $data = array(
                'app-email'                       => form_error('app-email'),
                'app-ehealthflex_market_purchase_code' => form_error('app-ehealthflex_market_purchase_code'),
            );
...
//BARCODE GEN IN PHP
    ...
    //     $this->load->view('admin/barcode_index.php');
    //     $this->load->view('layout/footer');
    // }

    // public function barcodeformcollect(){
    //     // var_dump($this->input->post()); die;
    //     $data['postData'] = $this->input->post();
    //     // $this->load->view('admin/barcode128.php',$data);

    //     $this->load->view('admin/barcode.php',$data);
	...
	public  function set_barcode($code)
	{
        ...
        // move_uploaded_file($imageBarcode, "./uploads/barcode/" . $barcode_image);

        $data['barcode_image'] = $barcode_image;
        $this->load->view('admin/print_ci3barcode', $data);
