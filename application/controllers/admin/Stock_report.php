...
class Stock_report  extends Admin_Controller
{
    ...
    public function stockreport()
    {
        ...
        $this->session->set_userdata('sub_menu', 'admin/stock/productwisestockreport');
        $data['title'] = 'title';
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($role_id == 4){         //FOR CHEMIST LOGIN
                $reportdata = $this->sales_model->getChemistProductwiseStockReport($date_from, $date_to,$loginuserId);
            }elseif($role_id == 73){    //FOR CENTRAL PHARMA ADMIN
                $reportdata = $this->purchase_model->getCentralPharmaProductwiseSalesReport($date_from, $date_to);
            ...
            $dt_data = array();
             ...
             if (!empty($reportdata)) {
                ...
                $row['mbd_qty'] = $value->mbd_qty;
                $row['mbd_availqty'] = $value->mbd_availqty;
        ...
        $this->load->view('admin/stock/productwisestockreport', $data);
