...
class Radio extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('radio_model','prefix_model', 'transaction_model'));  //radio model
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $deleted_parameter_array = array_diff($prev_inserted_array, $update_inserted_array);
             ...
             //na line for radio_para_details&pathology table
          ...
          if($medicine_uploaded_role_id != 6){
              $is_central_pharm = "no";
          }elseif($medicine_uploaded_role_id == 6){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($radiology_billing_id == '') {
                ...
                $transaction_data = array(
                    ...
                    // 'amount'            => $this->input->post('amount'),  //changed here
                    'amount'            =>0,
                    'type'              => 'payment',
...
//======
    ...
    {
        $this->load->helper('download');
        $filepath = "./backend/import/import_radiotest_sample_file.csv";
        $data     = file_get_contents($filepath);
        $name     = 'import_radiotest_sample_file.csv';
    ...
    {
        if (!$this->rbac->hasPrivilege('import_radio_test', 'can_view')) {
            access_denied();
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            $array = array('status' => 'fail', 'error' => $msg, 'message' => '');
            $this->load->view('layout/header');
            $this->load->view('admin/radio/import', $data);
            $this->load->view('layout/footer');
        } else {
            //na line for radio table import
            $medicine_uploaded_role = $this->customlib->getStaffRole();  //to get Staff Role
            $medicine_uploaded_role_id = json_decode($medicine_uploaded_role)->id; 
            if($medicine_uploaded_role_id != 6){
                $is_central_pharm = "no";
                ...
                if ($ext == 'csv') {
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            //         $insert_id = "";
                            //     } else {
                                    $insert_id = $this->radio_model->addImport($medicine_data[$i]);
                            //     }
                            // }
