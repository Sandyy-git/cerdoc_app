 ...
 */
    ...
    public function consult()
    {
        ...
        $data["doctors"]            = $doctors;
        $conference_setting         = $this->setting_model->getzoomsetting();
        // var_dump($conference_setting); die;
        $data['conference_setting'] = $conference_setting;
        $doctorid                   = "";
        $data['opd_ipd']            = $this->opd_ipd;
        $data["bloodgroup"]         = $this->bloodbankstatus_model->get_product(null, 1);
 ...
 */
  ...
  //na e
        ...
        $data['stafflist']       = $this->staff_model->getall('', 1,$r_id);
        $this->load->view('layout/header');
  ...
  //na e
    ...
    public function vpreport()
    {
        if (!$this->rbac->hasPrivilege('vp_report', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'zoom_conference/vpreport');
        $data                    = array();
        $staff_id                = $this->customlib->getStaffID();
        $data['logged_staff_id'] = $staff_id;
        $data['opd_ipd']         = $this->opd_ipd;
  ...
  //na e
        // $data['stafflist']       = $this->staff_model->getall('', 1,$r_id);
        $data['stafflist']       = $this->staff_model->getAllDoclogin('', 1,$r_id);
        $this->load->view('layout/header');
        $this->load->view('admin/conference/vp_report', $data);
        $this->load->view('layout/footer');
    }
    ...
    public function valuepointreport()
    {
        // echo "<pre>";
        ...
        $data['created_by']  = $this->input->post('created_by');
        $data['module_type'] = $this->input->post('select_module');
        $dt_response = $this->conference_model->getvpreports($data);
        // var_dump($dt_response); die;
        $dt_response = json_decode($dt_response);
        $dt_data     = array();
        $sum = 0;
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                //==============================
                $row[]     = $value->doctor_firstname.' '.$value->doctor_lastname;
                $row[]     = $value->pres_id;
                $row[]     = composePatientName($value->patient_name, $value->patient_id);
                $row[]     = $this->customlib->YYYYMMDDHisTodateFormat($value->bill_date, $this->customlib->getHospitalTimeFormat());
                $row[]     = $value->med_quantity;
                $row[]     = $value->pharm_bill_id;
                $row[]     = $value->med_vp;
                $row[]     = $value->doc_med_vp;
