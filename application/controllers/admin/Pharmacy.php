...
class Pharmacy extends Admin_Controller
{
       ...
       // $this->form_validation->set_rules('unit', $this->lang->line('unit'), 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $array = array('status' => 'fail', 'error' => $msg, 'message' => '');
             ...
             //na line for pharmacy from add table
 ...
 //na end
    ...
    {
        $dt_response = $this->pharmacy_model->getAllpharmacybillRecord();   //data load to admin pharmacy bill list
...
//na end
    ...
    public function getDetails()
    {
        ...
        if($uLogin == $result['added_by']){
            $result['can_edit'] = "yes";
        }else{
            $result['can_edit'] = "no";
        }
        echo json_encode($result);
        // echo json_encode($canEdit);
        ...
        if ($this->form_validation->run() == false) {
            $msg = array(
               ...
               // 'medicine_group'       => form_error('medicine_group'),
				...
				'valuep'       => form_error('valuep'),
				'loyalp'       => form_error('loyalp'),
...
//          $med_added_by = $this->customlib->getUserData();
//          $added_by = $med_added_by['id'];
//          var_dump($medicine_uploaded_role_id); die;
// //na end
    ...
    public function get_medicine_searchBy_name()
    {
        $medicine_category_id = $this->input->post("medicine_category_id");
        $data                 = $this->pharmacy_model->get_medicine_searchBy_name($medicine_category_id);
        echo json_encode($data);
    }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($action_type == 'update') {
                ...
                if ($payment_amount >= 0) {
                    ...
                    $payment_array = array(
                        // 'amount'       => $this->input->post('payment_amount'),  //changed here
                        'amount'       =>0,
                        'type'         => 'payment',
                        'patient_id'   => $patient_id,
                        'section'      => $payment_section['pharmacy'],
 ...
 //na e
        ...
        $staffsearch          = $this->patient_model->getstaffpharmacybill($r_id);
