...
class Lab extends Admin_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($lab_id)) {
                ...
                $data  = array('lab_name' => $labName,'added_by' => $added_by,'is_central_pharm' => $is_central_pharm);
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('update_message'));
            }
    ...
    public function addparameter()
    {
        ...
        //na line for radioparameter table
        $medicine_uploaded_role = $this->customlib->getStaffRole();  //to get Staff Role
        $medicine_uploaded_role_id = json_decode($medicine_uploaded_role)->id; 
        if($medicine_uploaded_role_id != 6){
            $is_central_pharm = "no";
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($parameter_id)) {
                ...
                $data  = array('parameter_name' => $parametername, 'reference_range' => $referencerange, 'description' => $description, 'unit' => $unit,'added_by' => $added_by,'is_central_pharm' => $is_central_pharm);
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('update_message'));
            }
