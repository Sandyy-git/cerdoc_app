...
class Pathology extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('pathology_model','prefix_model', 'transaction_model'));
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $deleted_parameter_array = array_diff($prev_inserted_array, $update_inserted_array);
              ...
              //na line for path_para_details&pathology table
            ...
            if($medicine_uploaded_role_id != 5){
                $is_central_pharm = "no";
            }elseif($medicine_uploaded_role_id == 5){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($pathology_billing_id == '0') {
                ...
                $transaction_data = array(
                    ...
                    // 'amount'               => $this->input->post('amount'),          //changing here
                    'amount'               =>  0,
                    'type'                 => 'payment',
    ...
    public function pathologyDetails()
    {
        ...
        //na to hide path test edit in other logins
        ...
        $data['result'] = $result;
        //na
        if($userLoggedIn == $result->added_by){
            $data_edit['can_edit'] = 'yes';
        }
        // elseif($userLoggedIn == 1){
        //     $data_edit['can_edit'] = 'yes';
        // }
        else{
            $data_edit['can_edit'] = 'no';
        }
        // var_dump($data); die;
        //nae
        $page = $this->load->view("admin/pathology/_pathologyDetails", $data, true);
        echo json_encode(array('status' => 1, 'page' => $page,'can_edd'=> $data_edit));
    }
    ...
    public function gettestreportbatch()
    {
        if (!$this->rbac->hasPrivilege('pathology_bill', 'can_view')) {
          ...
          //na line for restrivtion  table
          $med_added_by = $this->customlib->getUserData();
          $created_by = $med_added_by['created_by'];
          $data['view_add_med'] = $created_by;
  //na end
 ...
 //TO GET AJAXLIST IN PATH FIRST PAGE AJAXLIST
    ...
    //TO GET PATH TEST RECORD ON SECOND PAGE
...
//na for barcode
    public function getPathologyBarcodeDetail(){
        $id             = $this->input->post('id');
        $data['id']     = $id;
        ...
        $patient_barcode = $result->barcode;
        if($result->barcode == ""){
            $update = 1;
        }else{
            $update = 0;
        ...
        $patient_name = $result->patient_name." (".$result->patient_id.")";
        $patient_age = $this->customlib->getPatientAge($result->age,$result->month,$result->day);
        $patient_gender = $result->gender;

		$this->set_barcode($temp,$patient_name,$patient_age,$patient_gender,$update,$id,$patient_barcode);

        // $page = $this->load->view('admin/pathology/_print_ci3barcode', $data, true);
        // echo json_encode(array('status' => 1, 'page' => $page));
    }
//na for barcode
    public  function set_barcode($code,$patient_name,$patient_age,$patient_gender,$update,$id,$patient_barcode)
	{
        if($update == 1){
		//load library
		...
		//generate barcode
		// $imageBarcode = Zend_Barcode::render('code128', 'image', array('text'=>$code), array());
        $imageBarcode =  Zend_Barcode::draw('code128', 'image', array('text'=>$code),array());
        // var_dump($imageBarcode); die;
        $barcode_image = 'barcode-' . date('m-d-Y-H-i-s') . '.png';
        imagepng($imageBarcode, "./uploads/barcode/".$barcode_image);        
            $data = array(
                'barcode' => $barcode_image,
                );
            $result = $this->pathology_model->updatePathologyBillBarcode($data,$id);
        }else{
            $barcode_image= $patient_barcode;
        }
        ...
        // echo `<html><body><table><tr><td>`.print_r($data['result']  ).`</td></tr></table></body>/html>`;

        // echo "<p class='inline'><span ><b>Item: 'Sanky' </b></span>".bar128(stripcslashes('Monkey'))."<span ><b>Price: ".'sds'." </b><span></p>&nbsp&nbsp&nbsp&nbsp";
    ...
    {
        $this->load->helper('download');
        $filepath = "./backend/import/import_pathologytest_sample_file.csv";
        $data     = file_get_contents($filepath);
        $name     = 'import_pathologytest_sample_file.csv';
    ...
    {
        if (!$this->rbac->hasPrivilege('import_test', 'can_view')) {
            access_denied();
        }

        $check = $this->form_validation->set_rules('file', $this->lang->line('file'), 'callback_handle_csv_upload');

        // $medicineCategory         = $this->medicine_category_model->getMedicineCategory();
        // var_dump($medicineCategory);
        // $data["medicineCategory"] = $medicineCategory;
        $fields                   = array('parameter_name', 'reference_range', 'unit', 'description');
        $data["fields"]           = $fields;
        if ($this->form_validation->run() == false) {
            // var_dump($data); die;
            ...
            $array = array('status' => 'fail', 'error' => $msg, 'message' => '');
            $this->load->view('layout/header');
            $this->load->view('admin/pathology/import', $data);
            $this->load->view('layout/footer');
        } else {
             //na line for path table
             $medicine_uploaded_role = $this->customlib->getStaffRole();  //to get Staff Role
             $medicine_uploaded_role_id = json_decode($medicine_uploaded_role)->id; 
             if($medicine_uploaded_role_id != 5){
                 $is_central_pharm = "no";
 ...
 //na end
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                if ($ext == 'csv') {
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            //         $insert_id = "";
                            //     } else {
                                    $insert_id = $this->pathology_model->addImport($medicine_data[$i]);
                            //     }
                            // }
                ...
                redirect('admin/pathology/import');
            }
        }
    }
