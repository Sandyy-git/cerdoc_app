...
class Leavetypes extends Admin_Controller
{
    ...
    public function createLeaveType()
    {
        ...
        $data["title"] = $this->lang->line('add_leave_type');
         ...
         $lt_uploaded_role = $this->customlib->getStaffRole();  //to get Staff Role
         $lt_uploaded_role_id = json_decode($lt_uploaded_role)->id; 
         if($lt_uploaded_role_id != 7){
             $is_central_login = "no";
         }elseif($lt_uploaded_role_id == 7){
             $is_central_login = "yes";
         }
         $lt_added_by = $this->customlib->getUserData();
         $added_by = $lt_added_by['id'];
//na end
        ...
        if ($this->form_validation->run()) {
            ...
            if (!empty($leavetypeid)) {
                ...
                $data = array('type' => $type, 'is_active' => 'yes','added_by' => $added_by,'is_central_login' => $is_central_login);
            }
