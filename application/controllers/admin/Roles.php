...
class Roles extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if($role_id == 73){
            ...
            $data = array(
                ...
                'staff_roles' => $this->input->post('rolesStaff'),
            );
            $this->role_model->add($data); //insert into role
    ...
    {                                                                  //checks page will load
        ...
        $role_permission         = $this->role_model->find($role['id']);     //from permission_group table,condition >> sort_order and get permissions from permission_category & roles_permissions using id and role_id from permission_group
        $data['role_permission'] = $role_permission;                        
        if ($this->input->server('REQUEST_METHOD') == "POST") {

            $open_tab     = $this->input->post('open_tab');     //appended value comes from save 
