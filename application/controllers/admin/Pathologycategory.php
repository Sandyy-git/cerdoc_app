...
class Pathologycategory extends Admin_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($pathology_category_id)) {
                ...
                $data  = array('category_name' => $categoryName,'added_by' => $added_by,'is_central_pharm' => $is_central_pharm);
    ...
    public function addparameter()
    {
        ...
        //na line for pathparameter table
        $medicine_uploaded_role = $this->customlib->getStaffRole();  //to get Staff Role
        $medicine_uploaded_role_id = json_decode($medicine_uploaded_role)->id; 
        if($medicine_uploaded_role_id != 5){
            $is_central_pharm = "no";
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($pathology_parameter_id)) {
                ...
                $data  = array('parameter_name' => $parametername, 'reference_range' => $referencerange, 'unit' => $unit, 'description' => $description,'added_by' => $added_by,'is_central_pharm' => $is_central_pharm);
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
            }
    ...
    public function addunit()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($unit_id)) {
                ...
                $data  = array('unit_name' => $unitname, 'unit_type' => $unittype,'added_by' => $added_by,'is_central_pharm' => $is_central_pharm);
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
            }
            $insert_id = $this->pathology_category_model->addunit($data);
        }
