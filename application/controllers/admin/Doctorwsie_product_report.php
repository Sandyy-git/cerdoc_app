...
class Doctorwsie_product_report extends Admin_Controller
{
    ...
    public function docwiseproductreport()
    {
        ...
        $this->session->set_userdata('sub_menu', 'admin/doctorwisereport/doctorwiseproductreport');
        $data['title'] = 'title';
        $data['locality'] = $this->staff_model->getStaffLocality();

        // $this->form_validation->set_rules('localityid', $this->lang->line('staff_locality'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            $msg = array(
                // 'localityid' => form_error('localityid'),
            ...
            if($role_id == 73){    //FOR CENTRAL PHARMA ADMIN
                $reportdata = $this->staff_model->getDocwiseProReport($date_from, $date_to,$doctorid );
        ...
        $this->load->view('admin/doctorwisereport/doctorwiseproductreport', $data);
