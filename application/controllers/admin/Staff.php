...
//na end
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $resultlist          = $this->staff_model->getEmployee($role, 1,$role_id);
                        $data['resultlist']  = $resultlist;
                    }
    ...
    public function disablestafflist()
    {
        ...
        //na changed above line to below
        $staffRole     = $this->customlib->getStaffRole();
        $role_id = json_decode($staffRole)->id;  
        //na ends

         //na
         if($role_id!=7) {
             $data["roles"] = $this->role_model->getRoleFromStaffid($role_id);
         }else{
 ...
 //na end
        ...
        //na changed above line to below
        $resultlist         = $this->staff_model->searchFullText($search_text, 0,$role_id);
    //na end
        $data['resultlist'] = $resultlist;
        ...
        if (isset($search)) {
            if ($search == 'search_filter') {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $data['search_text'] = $this->input->post('search_text');
                    //na
                    $resultlist          = $this->staff_model->getEmployee($role, 0,$role_id);
                    $data['resultlist']  = $resultlist;
                }
            } else if ($search == 'search_full') {
                $data['searchby']    = "text";
    ...
    public function create()
    {
        ...
        //na 
        // $this->form_validation->set_rules('latitude', "Please allow location".",".$this->lang->line('latitude')." "."and"." ".$this->lang->line('longitude'), 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            //get and save default setting for clinic if the role is not system ..means we created it
            // if role is system  ---check who is creating this  --if not super admin then insert parent  as our role parent id
            // parent_id ,staff_id ---both are same if
            $insert = false;
            $current_user_staff_id = $this->customlib->getStaffID();

            if($this->input->post('role') <= 7 && $current_user_role_id >7){
                $parentStaffId = $current_user_staff_id;
                $staff_id = $staff_id;
                $parentchild  = array(
                    'parent_staff_id' => $parentStaffId,
                    'child_staff_id' => $staff_id,
                );

            }else{
                $staff_id = $staff_id;
                $parentStaffId = $staff_id;
                $parentchild  = array(
            ...
            //insert in parent child table
            $this->db->insert('parent_child',$parentchild);
            $setting_result         = $this->setting_model->get();
            $settingData = array(
                'parent_staff_id'                       => $parentStaffId,
                'staff_id'                              => $staff_id,
                'name'                     => "Change to your clinic name",
                'phone'                    => "Change to your clinic phone number",
                'dise_code'                => "Change to your clinic code",
                'address'                  => "Change to your clinic address",
                'email'                    => "Change to your clinic email address",
    ...
    public function edit($id)
    {
        ...
        if($this->input->post("role")=="3" && $this->input->post("local_identification_number")==""){
            $this->form_validation->set_rules('local_identification_number', "Medical Registration ", 'trim|required|xss_clean');
    ...
    public  function insertStaffidtoprinting($staffId){
        $data = array(
            array(
                'staff_id' => $staffId ,
                'print_header' => 'uploads/printing/1.jpg' ,
                'print_footer' => 'Change this to whatever you wants',
                'setting_for' => 'opdpre',
                'is_active' => 'yes'
            ),
            array(
        ...
        $this->db->insert_batch('print_setting', $data);
    }

    public function getcredentialdatatable()
    {
        $dt_response = $this->staff_model->getAllcredentialRecord();
        $dt_response = json_decode($dt_response);
        $dt_data     = array();
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                //====================
                $row[] = $value->uid;
                $row[] = $value->clinic_name;                 //newly added
                $row[] = $value->role_name;                 //newly added
    ...
    public function adminCredentialReport()
    {
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/staff/admincredentialreport');
        $this->load->view("layout/header");
