        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($check->num_rows() > 0) {
                    ...
                    //na to hide add pres in super admin
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($check->num_rows() > 0) {
                    ...
                    if ($this->session->has_userdata('hospitaladmin')) {
                        $doctor_restriction = $this->session->userdata['hospitaladmin']['doctor_restriction'];
                        // var_dump($doctor_restriction); die;
                        if ($doctor_restriction == 'enabled') {
                            if ($userdata["role_id"] == 3) {
...
//na to hide add pres in 2 tab of visit
                        if ($doctor_restriction == 'disabled') {
...
//na e
                        ...
                        //na to hide add pres in super admin on sec visit tab
                    if ($userdata["role_id"] == 7) {
                        $result[$key]['prescription'] = 'not_applicable';
                    }
                    //na e
...
//SECOND OVERVIEW PAGE
    public function visitdetails($id, $opdid)
    {
        if (!empty($id)) {
            ...
            // var_dump($result['investigations']); die;
...
//First Overview page
    public function profile($id)
    {
        ...
        if($doctorid != null) {
            $data['patientdetails'] = $this->patient_model->getpatientoverview($id,$doctorid);
        }else{
 ...
 public function yearchart()
    ...
    public function deactivePatient()
    {
        ...
        //     access_denied();
        // }
        if (!$this->rbac->hasPrivilege('enabled_disabled', 'can_view')) {   //changed
            access_denied();
        }
    ...
    public function add_opd_prescription()
    {
        ...
        if(!empty($data["printing_list"])){
            $img_path = $data["printing_list"]['print_header'];
        }
        //ends
                ...
                foreach ($total_rows as $row_key => $row_value) {
                    ...
                    if (isset($ipd_prescription_detail_id)) {
                        ...
                        $update_medicines[]         = array(
                            ...
                            'dose_duration_id' => $this->input->post("duration_dosage_" . $row_value),
                            'instruction'      => $this->input->post("instruction_" . $row_value),
                            'afbf'      => $this->input->post("afbf_" . $row_value),
