...
class Chemist_purchase_report extends Admin_Controller
{
    ...
    public function chemistpurchasereport()
    {
        if (!$this->rbac->hasPrivilege('chemist_purchase_report', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/sales/salesreport');
        $data['title'] = 'title';
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($role_id == 4){
                $reportdata = $this->sales_model->getChemistPurchaseReport($date_from, $date_to,$loginuserId);
            }elseif($role_id == 73){    //FOR CENTRAL PHARMA ADMIN
                $reportdata = $this->sales_model->getChemistPurchaseReporttoCP($date_from, $date_to);
        ...
        $this->load->view('admin/sales/chemistsalesreport', $data);
