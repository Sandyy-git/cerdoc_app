...
class Purchase_report extends Admin_Controller
{
    ...
    public function purchasereport()
    {
        ...
        $this->session->set_userdata('sub_menu', 'admin/purchase/purchasereport');
        $data['title'] = 'title';
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($role_id == 73){    //FOR CENTRAL PHARMA ADMIN
                $reportdata = $this->purchase_model->getTransactionBetweenDateLoginwisetoCp($date_from, $date_to);
            }elseif($role_id == 7){    //FOR SUPER ADMIN
                $reportdata = $this->purchase_model->getTransactionBetweenDateLoginwisetoCp($date_from, $date_to);
            }else{
                $reportdata = $this->purchase_model->getTransactionBetweenDateLoginwise($date_from, $date_to,$loginuserId);
            }            
            // echo "<pre>";
            ...
            $dt_data = array();
             ...
             if (!empty($reportdata)) {
                ...
                $row['bill_id'] = $value->bill_id;
                $row['tax_cgst']  = $value->tax_cgst;
                $row['tax_sgst']  = $value->tax_sgst;
                $row['tax']  = $value->tax;
                $row['bill_amount']  = $value->bill_amount;
                $dt_data[]                  = $row;
                }
                $data['result'] = $dt_data;
            }
        ...
        $this->load->view('admin/purchase/purchasereport', $data);
