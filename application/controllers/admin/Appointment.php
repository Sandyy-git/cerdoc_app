...
class Appointment extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        // var_dump($data["appoint_priority_list"] ); die;
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $check_for_alloted_slot = $this->appointment_model->check_alloted_slot($appointment);
            if(empty($check_for_alloted_slot)){
              
            $insert_id = $this->appointment_model->add($appointment);  //save to appointment table,, only upto appointment table
            $payment_data = array(
                'appointment_id' => $insert_id,               //from appointment table last insert id
                'paid_amount'    => $this->input->post('amount'),       
                'charge_id'      => $this->input->post('charge_id'),  //at first while adding transation_id payment_mode null
                'payment_type'   => 'Offline',
                'date'           => date("Y-m-d H:i:s"),
            );
            //upto appointment table only just collect array data given below
            // Array([appointment_id] => 366[paid_amount] => 530.00[charge_id] => 1[payment_type] => Offline[date] => 2022 - 09 - 08 13: 50: 27)
           
            $payment_section   = $this->config->item('payment_section');  //payment section from config dile
            
            $transaction_array = array(
                'amount'         => $this->input->post("amount"),     //here amount is going as 0 while adding appointment
                'patient_id'     => $patient_id,
                'section'        => $payment_section['appointment'],  //is very important
                'type'           => 'payment',
                'appointment_id' => $insert_id,                       // from appointment table last insert id
                'payment_mode'   => $this->input->post("payment_mode"),  //important
                'payment_date'   => date('Y-m-d H:i:s'),
                'received_by'    => $staff_id,
            );
            //upto appointment table only just collect array data given below
            // Array ( [amount] => 530.00 [patient_id] => 11 [section] => Appointment [type] => payment [appointment_id] => 368 [payment_mode] => Online [payment_date] => 2022-09-08 13:57:34 [received_by] => 84 )
            ...
            //saving to appointment_payment and transaction table
            //upto appointment,appointment_payment and transaction table data is going 
            //with null case_ref_id,visit_details_id on appointment table
            //with null case_ref_id,opd_id, amount going as 530 first  
            $this->appointment_model->saveAppointmentPayment($payment_data, $transaction_array);   //save to both appointment_payment and transaction table
        ...
        if($this->input->post('appointment_status') == 'approved'){
            ...
            $array = array('status' => 'fail', 'error' => 'error', 'message' => $this->lang->line('slot_booked'));
    }
        }
    ...
    {                                                    //sending data to modal to add appointment pending to approve based on id in appointment table
...
//na e
        ...
        //, and r_id added
        $doctorlist                    = $this->staff_model->getEmployeeByRoleID(3,1,$r_id);
    ...
    public function reschedule()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $check_for_alloted_slot = $this->appointment_model->reschedule_check_alloted_slot($appointment,$doctor,$approved_static);
            if(empty($check_for_alloted_slot) || $this->input->post('edit_appointment_status') == 'cancel' || $this->input->post('edit_appointment_status') == 'pending'){
            // if(empty($check_for_alloted_slot)){
            ...
            $transaction_data    = $this->transaction_model->getTransactionByAppointmentId($appointment_id);
            $appointment_payment = $this->appointment_model->getPaymentByAppointmentId($appointment_id);        
            // var_dump($appointment_details['visit_details_id']); die;
...
////////////////////////
            ...
            //down line is from add appointment
            // $opd_visit_id = $this->appointment_model->moveToOpd($opd_details, $visit_details, $charge, $appointment_id,'');
//down is original
...
//completed here
            ...
            $this->appointment_model->updatepapointmentcharges($opd_visit_id, $doctor_fees );
            $this->appointment_model->updatepapointmenttransactions($opd_visit_id);
