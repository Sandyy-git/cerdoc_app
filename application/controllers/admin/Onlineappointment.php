...
class Onlineappointment extends Admin_Controller
{
    ...
    public function doctorGlobalShift()
    {
        ...
        if($role_id == 3){
        $id = $this->customlib->getUserData();  //na
        $stor_id = $id['id'];
        $shift = $this->onlineappointment_model->globalDoctorShift($stor_id);//DOCTORS OVER
        // echo "<pre>";
        // print_r($shift); die;
        ...
        // print_r($shift); die;
        $data['shift']        = $shift;
//DOCTOR OVER CLINIC STARTS
        }elseif($role_id != 7){

        $tot_roles = $this->onlineappointment_model->getRoleFromStaffid($role_id);
        foreach ($tot_roles as $key => $value) {
            // var_dump($value["id"]); die;
                $count_roles[$value["name"]] = $this->onlineappointment_model->count_roles($value["id"],$role_id);
        }
        
        foreach ($count_roles['Doctor'] as $key => $doctor) {
            $shift[$key] = $this->onlineappointment_model->globalDoctorShift($doctor['id']);//CLINICS OVER
        }

        $keys = array_keys($shift);
        for($i = 0; $i < count($shift); $i++) {
        // echo $keys[$i] . "{<br>";
        foreach($shift[$keys[$i]] as $key_r => $r_value) {
        // echo $key_r . " : " . $r_value['id'] . "<br>";
        $shift[$keys[$i]]["doctor_shift"] = $this->onlineappointment_model->getGlobalDoctorShift($r_value['id']); //CLINICS OVER
        }
        // echo "}<br>";
        }
        // echo "<pre>";
        // print_r($shift); die;
        $data['clinic_shift']        = $shift;
        }elseif($role_id == 7){
            $shift = $this->onlineappointment_model->globalDoctorShiftAll();//DOCTORS OVER
            foreach ($shift as $shift_key => $shift_value) {
