...
class Schsettings extends Admin_Controller
{
    ...
    public function ajax_schedit()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $returnData = $this->setting_model->add($data);
            $this->load->helper('lang');
            $this->session->userdata['hospitaladmin']['date_format']            = $returnData[0]['date_format'];
            $this->session->userdata['hospitaladmin']['currency_symbol']        = $returnData[0]['currency_symbol'];
            $this->session->userdata['hospitaladmin']['timezone']               = $returnData[0]['timezone'];
            $this->session->userdata['hospitaladmin']['theme']                  = $returnData[0]['theme'];
            $this->session->userdata['hospitaladmin']['credit_limit']           = $returnData[0]['credit_limit'];
            $this->session->userdata['hospitaladmin']['opd_record_month']       = $returnData[0]['opd_record_month'];
            $this->session->userdata['hospitaladmin']['doctor_restriction']     = $returnData[0]['doctor_restriction'];
            $this->session->userdata['hospitaladmin']['superadmin_restriction'] = $returnData[0]['superadmin_restriction'];
            set_language($returnData[0]['lang_id']);
