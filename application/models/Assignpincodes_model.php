...
class Assignpincodes_model extends MY_model
{
    ...
    public function getStaffPincode($id = null)
    {
        $userLoggedInFirst = $this->customlib->getLoggedInUserId();
        $userRoleIn = $this->role_model->getRoleFromStaffUsingLid($userLoggedInFirst);
        ...
        if (!empty($id)) {
            $query = $this->db->join('staff_roles','staff_roles.staff_id=staff.id')->where("staff.id", $id)->where("staff.created_by",73)->where("staff_roles.role_id!=",4)->get('staff');
            ...
            $query = $this->db->join('staff_roles','staff_roles.staff_id=staff.id')->where("created_by",73)->where("staff_roles.role_id!=",4)->get("staff");
    ...
    public function delete($id)
    {
        ...
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where("id", $id)->delete("staff_pincode");
    ...
    public function deleteDisPincode($id){
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where("id", $id)->delete("distributor_pincodes");
        $message = DELETE_RECORD_CONSTANT . " On Medicine Category id " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
...
public function addPincode($data)
{
    ...
    if (isset($data['id'])) {
        $this->db->where('id', $data['id']);
        $this->db->update('staff_pincode', $data);
        $message = UPDATE_RECORD_CONSTANT . " On Medicine Dosage id " . $data['id'];
        $action = "Update";
        $record_id = $data['id'];
        $this->log($message, $record_id, $action);
    } else {
        $this->db->insert('staff_pincode', $data);
        $insert_id = $this->db->insert_id();
...
public function addPincodetoDis($data)
{
    $this->db->trans_start(); # Starting Transaction
    $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
    //=======================Code Start===========================
    if (isset($data['id'])) {
        $this->db->where('id', $data['id']);
        $this->db->update('distributor_pincodes', $data);
        $message = UPDATE_RECORD_CONSTANT . " On Medicine Dosage id " . $data['id'];
        $action = "Update";
        $record_id = $data['id'];
        $this->log($message, $record_id, $action);
    } else {
        $this->db->insert('distributor_pincodes', $data);
        $insert_id = $this->db->insert_id();
        $message = INSERT_RECORD_CONSTANT . " On Medicine Dosage id " . $insert_id;
        $action = "Insert";
        $record_id = $insert_id;
...
public function pincodestodis($id = null)
    {
        
        $userLoggedIn = $this->customlib->getLoggedInUserId();
        if (!empty($id)) {
            $query = $this->db->select('distributor_pincodes.*,staff.name')
                ->join('staff', 'staff.id = distributor_pincodes.staff_id','left')
                ->where('distributor_pincodes.id', $id)
                ->get('distributor_pincodes');
            return $query->row_array();
        } else {
            if($userLoggedIn == 7){
    ...
    public function getStaffPin($id = null)
    {
        $userLoggedIn = $this->customlib->getLoggedInUserId();
        if (!empty($id)) {
            $query = $this->db->select('staff_pincode.*')
                ->where('staff_pincode.id', $id)
                ->get('staff_pincode');
            return $query->row_array();
        } else {
    ...
    public function getStaffpcode($id = null)
    {
        $userLoggedIn = $this->customlib->getLoggedInUserId();
        if (!empty($id)) {
            $query = $this->db->select('staff_pincode.*')
    ...
    public function getDispcode($id = null)
    {
        $userLoggedIn = $this->customlib->getLoggedInUserId();
        if (!empty($id)) {
            $query = $this->db->select('distributor_pincodes.*')
                ->join('staff','staff.id = distributor_pincodes.staff_id', 'left')
                ->where('distributor_pincodes.id', $id)
                ->get('distributor_pincodes');
            return $query->row_array();
            ...
            $query = $this->db->select('distributor_pincodes.*')
                ->join('staff.id','staff.id = distributor_pincodes.staff_id','left')
                ->get('distributor_pincodes');
            return $query->result_array();
        }
    }
    ...
    public function check_pincode_exists($pincode)
    {
        $this->db->where(array('pincode' => $pincode));
        $query = $this->db->get('staff_pincode');
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    public function check_pincode_exists_dis($pincode)
    {
        $this->db->where(array('pincode' => $pincode));
        $query = $this->db->get('distributor_pincodes');
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
