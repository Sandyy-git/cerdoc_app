...
class Designation_model extends MY_model
{
    ...
    public function getAlldesignationRecord()
    {
        ...
        $this->datatables
            ...
            if($userLoggedIn == 7){
                $this->datatables->where('staff_designation.added_by',$userLoggedIn);
            }else{
                $this->datatables->where('staff_designation.added_by',$userLoggedIn);
                // $this->datatables->or_where('staff_designation.is_central_login','yes');
            }

            $this->datatables->from('staff_designation');
