...
{
//==============================================================================
    public function check_alloted_slot($appointment){
        $this->db->select('date,doctor,shift_id');
    ...
    public function reschedule_check_alloted_slot($appointment,$doctor,$approved_static){
        $this->db->select('date,doctor,shift_id');
        $this->db->where('appointment.date', $appointment["date"]);
        $this->db->where('appointment.shift_id', $appointment["shift_id"]);
        $this->db->where('appointment.doctor', $doctor);
        $this->db->where('appointment.appointment_status', $approved_static);
...
//========================================================================================
    public function add($appointment)                    //add appointment return id
    {
...
//=========================================================================================
    ...
    public function getAllappointmentRecord()
    {
        ...
        if (!empty($custom_fields)) {
            ...
            if ($userdata["role_id"] != 3 && $userdata["role_id"] != 7) {                          //newly added  //in clinic appointment shown related to only that doc
                $this->datatables->where('staff.created_by', $userdata['role_id']);
            }
...
//=========================================================================================
    ...
    public function saveAppointmentPayment($payment_data, $transaction)       //save appointment_payment and transsaction
    {
        $this->db->trans_start();
    ...
    public function moveToOpd($opd_details, $visit_details, $charges, $appointment_id, $doctor_fees)
    {
        ...
        $transaction_data['amount']            = $doctor_fees;       //changed here
        // $transaction_data['amount']            = '0';
        $this->db->update("transactions", $transaction_data, array("appointment_id" => $appointment_id));  //transactiontable update      
        $appointment_data['case_reference_id'] = $case_id;
        $appointment_data['visit_details_id']  = $visit_details_id;
        $this->db->update("appointment", $appointment_data, array("id" => $appointment_id));  //appoin table update
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
            return false;
    ...
    public function updatepapointmentcharges($opd_visit_id, $doctor_fees)
    {        
        $data['amount'] = $doctor_fees;            
        $this->db->where('opd_id', $opd_visit_id);
        $this->db->update('patient_charges', $data);     
        
    }

    public function updatepapointmenttransactions($opd_visit_id)
    {        
        $data['amount'] = '0';            
        $this->db->where('opd_id', $opd_visit_id);
