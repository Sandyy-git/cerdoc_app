...
class Pathology_model extends MY_Model
{
 ...
 //TO GET AJAXLIST IN PATH FIRST PAGE
    public function getAllpathologybillRecord()
    {
        ...
        $this->datatables
            ...
            //na
            ->where_in('pathology_billing.generated_by', $coll_array)
//na e
        ...
        $this->datatables
            ...
            ->sort('pathology_billing.id', 'desc')
            //na
            ->where('pathology_billing.generated_by', $login_user_id)
            //na e
            ->from('pathology_billing');
        return $this->datatables->generate('json');
    ...
    {
    //na addded to show pat test
    $user_loginId = $this->customlib->getLoggedInUserID();
        $i                         = 1;
       $custom_fields             = $this->customfield_model->get_custom_fields('pathologytest', 1);
        ...
        $this->datatables
           ...
           ->join('tax_category', 'tax_category.id = charges.tax_category_id', 'left')
            ...
            if($user_loginId != 1){
                $this->datatables->where('pathology.added_by', $user_loginId);
                // $this->datatables->or_where('pathology.is_central_pharm', 'yes');
            }
            $this->datatables->from('pathology');
        return $this->datatables->generate('json');
    }
    ...
    public function getpathotestDetails()
    {
        ...
        //na
        $this->db->where('pathology.added_by', $staffRoles_id);
        // $this->db->or_where('pathology.is_central_pharm', 'yes');
        //ne

         }elseif($login_user_id == 0){
            $this->db->select('pathology.*,pathology_category.id as category_id,pathology_category.category_name,charges.id as charge_id, charges.name, charges.charge_category_id, charges.standard_charge, charges.description,tax_category.percentage as tax');
            $this->db->join('pathology_category', 'pathology.pathology_category_id = pathology_category.id', 'left');
            $this->db->join('charges', 'pathology.charge_id = charges.id', 'left');
            $this->db->join('tax_category', 'charges.tax_category_id = tax_category.id', 'left');
...
//na update barcode model
    public function updatePathologyBillBarcode($data,$id){
        $this->db->update('pathology_report',$data,"id='$id'");
    }
    ...
    public function getPathology($id = null)
    {
        ...
        if (!empty($id)) {
            ...
            if($userLoggedIn == 7){
                $this->db->where('pathology.added_by',$userLoggedIn);
            }else{
                $this->db->where('pathology.added_by',$userLoggedIn);
                // $this->db->or_where('pathology.is_central_pharm','yes');
            }
    ...
    public function addImport($medicine_data)
    {
        ...
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->insert('pathology_parameter', $medicine_data);
        $insert_id = $this->db->insert_id();
