...
class Conference_model extends MY_Model
{
    ...
    public function getvpreports($data) {
        //    var_dump($data); die;
        $condition="";

        if($data['created_by']!=""){
            $condition.=" where staff.id= '".$data['created_by']."' " ;
        }
        ...
        if(isset($data['start_date']) && $data['start_date']!="" && isset($data['end_date']) && $data['end_date']!="" ){
            $start_date = $data['start_date'] ;
            $end_date = $data['end_date'] ;
            $condition.= " and date_format(pharmacy_bill_basic.date,'%Y-%m-%d ') >='". $start_date."' and date_format(pharmacy_bill_basic.date,'%Y-%m-%d') <= '".$end_date."' " ;
        }   
    
        $sql="SELECT staff.name as doctor_firstname,staff.surname as doctor_lastname,pharmacy.medicine_name as med_name,pharmacy.valuep as med_vp,pharmacy_bill_detail.quantity as med_quantity,(pharmacy.valuep * pharmacy_bill_detail.quantity) as doc_med_vp,patients.patient_name as patient_name,patients.id as patient_id,ipd_prescription_basic.id as pres_id,pharmacy_bill_basic.id as pharm_bill_id,pharmacy_bill_basic.date as bill_date from staff
          JOIN `ipd_prescription_basic`  ON `ipd_prescription_basic`.`prescribe_by` = `staff`.`id`
          JOIN `pharmacy_bill_basic`  ON `pharmacy_bill_basic`.`ipd_prescription_basic_id` = `ipd_prescription_basic`.`id`
          JOIN `pharmacy_bill_detail`  ON `pharmacy_bill_detail`.`pharmacy_bill_basic_id` = `pharmacy_bill_basic`.`id`

          JOIN `medicine_batch_details`  ON `medicine_batch_details`.`id` = `pharmacy_bill_detail`.`medicine_batch_detail_id`
          JOIN `pharmacy`  ON `pharmacy`.`id` = `medicine_batch_details`.`pharmacy_id`

          JOIN patients on patients.id = pharmacy_bill_basic.patient_id
             ".$condition." ";
           $this->datatables->query($sql) 
            //   ->searchable('conferences.title')
            //   ->orderable('"",conferences.title,patients.patient_name,conferences.date,conferences.api_type,conferences.created_id,total_viewers')
    ...
    public function getAllconsultRecord($staff_id = null)
    {
        ...
        if($role_id == 66 || $role_id == 7){
            $this->datatables->where('conferences.created_id', $staff_id);
        }else{
            if ($staff_id != "") {
