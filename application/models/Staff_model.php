...
class Staff_model extends MY_Model
{
    ...
    public function getAll($id = null, $is_active = null,$r_id)
    {
        ...
        if($r_id != ''){
            // echo "first";
            $this->db->where('staff.created_by', $r_id);
        }
        //na e
    ...
    public function getAllDoclogin($id = null, $is_active = null,$r_id)
    {
        if($r_id == 3){
    ...
    public function getEmployeeByRoleID($role, $active = 1,$r_id)
    {
        if($r_id != '7'){
            //append
        $query = $this->db->select("staff.*,staff_designation.designation,department.department_name as department, roles.id as role_id, roles.name as role")->join('staff_designation', "staff_designation.id = staff.staff_designation_id", "left")->join('staff_roles', "staff_roles.staff_id = staff.id", "left")->join('roles', "roles.id = staff_roles.role_id", "left")->join('department', "department.id = staff.department_id", "left")->where("staff.is_active", $active)->where("staff.created_by", $r_id)->where("roles.id", $role)->order_by('staff.name')->get("staff");
        }else{
        $query = $this->db->select("staff.*,staff_designation.designation,department.department_name as department, roles.id as role_id, roles.name as role")->join('staff_designation', "staff_designation.id = staff.staff_designation_id", "left")->join('staff_roles', "staff_roles.staff_id = staff.id", "left")->join('roles', "roles.id = staff_roles.role_id", "left")->join('department', "department.id = staff.department_id", "left")->where("staff.is_active", $active)->where("roles.id", $role)->order_by('staff.name')->get("staff");
        }
        return $query->result_array();
    ...
    public function getEmployeeByRoleIDNew($role, $active = 1)
    {
        $query = $this->db->select("staff.*,staff_designation.designation,department.department_name as department, roles.id as role_id, roles.name as role")->join('staff_designation', "staff_designation.id = staff.staff_designation_id", "left")->join('staff_roles', "staff_roles.staff_id = staff.id", "left")->join('roles', "roles.id = staff_roles.role_id", "left")->join('department', "department.id = staff.department_id", "left")->where("staff.is_active", $active)->where("roles.id", $role)->order_by('staff.name')->get("staff");
        return $query->result_array();
    }
   ...
   public function getStaffSpeciality($id)
    ...
    public function searchFullText($searchterm, $active,$role_id)
    {
        $i                         = 1;
    ...
    public function getImageId($staff_id)
    {
        $data  = array('id' => $staff_id);
        $query = $this->db->select('created_by')->where($data)->get("staff");
        return $query->row_array();
    }
    ...
    public function getAllcredentialRecord()
    {
        ...
        $this->datatables
            // ->select('staff.id as uid,staff.name,staff.email,staff.password')
            ->select('staff.id as uid,staff.name,staff.email,staff.password,sch_settings.name as clinic_name,roles.name as role_name')
            ->join('sch_settings', 'staff.id = sch_settings.staff_id','left')
            ->join('roles', 'staff.created_by = roles.id','inner')
            // ->searchable('staff.id,staff.name,staff.email')
            // ->orderable('staff.id,staff.name,staff.email')
            // ->sort('staff.id', 'desc')
            ->sort('staff.created_by', 'asc')
            ->from('staff');
    ...
    public function getNetworkStaffs(){
        $userdata           = $this->customlib->getUserData();
        $staff_id            = $userdata['id'];
        //select parent_staff_id from parent_child where child_staff_id= $staff_id
        $this->db->select('parent_child.parent_staff_id ');
        $this->db->from('parent_child');
        $this->db->where('child_staff_id', $staff_id);
        $query = $this->db->get();
        $parentId = $query->row_array();

        $this->db->select('parent_child.child_staff_id ');
        $this->db->from('parent_child');
        $this->db->where('parent_staff_id', $parentId['parent_staff_id']);
        $query  = $this->db->get();

        $ids = array();
        foreach ($query->result_array() as $id)
        {
            $ids[] = $id['child_staff_id'];
        }

        $query = $this->db->select("staff.*")->where_in("staff.id", $ids)->order_by("staff.name")->get("staff");
        $result =  $query->result_array();
        return $result;
    }

    public function getNetworkStaffsCp(){
        $userdata           = $this->customlib->getUserData();
        $staff_id            = $userdata['id'];
        //select parent_staff_id from parent_child where child_staff_id= $staff_id
        $this->db->select('parent_child.parent_staff_id ');
        ...
        $this->db->select('parent_child.child_staff_id ');
        $this->db->join('staff', 'staff.id = parent_child.parent_staff_id','left');
        $this->db->join('staff_roles', 'staff_roles.staff_id = staff.id','left');


        $this->db->from('parent_child');
        $this->db->where('parent_child.parent_staff_id', $parentId['parent_staff_id']);
        $this->db->or_where('staff.created_by', 7);
