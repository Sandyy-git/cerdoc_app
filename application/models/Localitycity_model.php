...
class Localitycity_model extends MY_model
{
    ...
    public function getLocalityCity($id = null)
    {
        $userLoggedInFirst = $this->customlib->getLoggedInUserId();
        $userRoleIn = $this->role_model->getRoleFromStaffUsingLid($userLoggedInFirst);
    ...
    public function delete($id)
    {
        ...
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where("id", $id)->delete("locality_city");
...
public function addLocalityCity($data)
{
    $this->db->trans_start(); # Starting Transaction
    $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
    //=======================Code Start===========================
    if (isset($data['id'])) {
        $this->db->where('id', $data['id']);
        $this->db->update('locality_city', $data);
        $message = UPDATE_RECORD_CONSTANT . " On Medicine Dosage id " . $data['id'];
        $action = "Update";
        $record_id = $data['id'];
        $this->log($message, $record_id, $action);
    } else {
        $this->db->insert('locality_city', $data);
        $insert_id = $this->db->insert_id();
...
public function getCity($id = null)
    {
        $userLoggedIn = $this->customlib->getLoggedInUserId();
        if (!empty($id)) {
            $query = $this->db->select('locality_city.*,staff_locality.locality')
                ->join('staff_locality', 'staff_locality.id = locality_city.locality_id')
                ->where('locality_city.id', $id)
                ->get('locality_city');
            return $query->row_array();
        } else {
