...
class Symptoms_model extends MY_Model
{
    ...
    public function get($id = null)
    {
        ...
        if (!empty($id)) {
            ...
            if($userLoggedIn == 7){
                $this->db->where('symptoms.added_by',$userLoggedIn);
            }else{
                $this->db->where('symptoms.added_by',$userLoggedIn);
                $this->db->or_where('symptoms.is_central_login','yes');
            }
            $this->db->from('symptoms');
            $this->db->join('symptoms_classification', 'symptoms_classification.id = symptoms.type', 'left');
            $query = $this->db->get();
    ...
    public function getsymtype($id = null)
    {
        ...
        if ($id != null) {
            ...
            if($userLoggedIn == 7){
                $this->db->where('symptoms_classification.added_by',$userLoggedIn);
            }else{
                $this->db->where('symptoms_classification.added_by',$userLoggedIn);
                $this->db->or_where('symptoms_classification.is_central_login','yes');
            }
