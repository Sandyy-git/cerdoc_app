...
{    
   ...
   public function ipdopdPaymentByCaseId($case_id)
    ...
    //FOR BOTH SAME
    public function getTransactionBetweenDateLoginwise($start_date, $end_date,$transaction_type='all',$r_id,$transstaffArray) {
        $this->db->select('transactions.*,staff.name,staff.surname,staff.employee_id,opd_details.generated_by as opd_bill_by,pharmacy_bill_basic.generated_by as pharm_bill_by,pathology_billing.generated_by as path_bill_by,radiology_billing.generated_by as radio_bill_by')->from('transactions');
        // $this->db->select('transactions.*,opd_details.generated_by')->from('opd_details');
        if ($transaction_type == "all") {
        }elseif ($transaction_type == "payment") {
           $this->db->where('transactions.type', $transaction_type);
        }elseif ($transaction_type == "refund") {
        ...
        if($r_id != '7'){
                        $this->db->where_in('opd_details.generated_by',$transstaffArray);
                        $this->db->or_where_in('pharmacy_bill_basic.generated_by',$transstaffArray);
                        $this->db->or_where_in('pathology_billing.generated_by',$transstaffArray);
                        $this->db->or_where_in('radiology_billing.generated_by',$transstaffArray);

            // $this->db->where('opd_details.generated_by', $r_id);
            // $this->db->or_where('pharmacy_bill_basic.generated_by', $r_id);
            // $this->db->or_where('pathology_billing.generated_by', $r_id);
            // $this->db->or_where('radiology_billing.generated_by', $r_id);
        }

        $this->db->join('staff', 'staff.id = transactions.received_by','left');
        $this->db->join('opd_details', 'opd_details.id = transactions.opd_id','left');
        $this->db->join('pharmacy_bill_basic', 'pharmacy_bill_basic.id = transactions.pharmacy_bill_basic_id','left');
        $this->db->join('pathology_billing', 'pathology_billing.id = transactions.pathology_billing_id','left');
        $this->db->join('radiology_billing', 'radiology_billing.id = transactions.radiology_billing_id','left');


        $this->db->where('DATE(transactions.payment_date) >=', $start_date);
        $this->db->where('DATE(transactions.payment_date) <=', $end_date);
...
//new function
    ...
    public function add($data)
    {
        ...
        if($role_id!=7) { //means it is not super admin, so take parent id of staff id
             $parentid =  $this->getParentStaffId($staffid);
        }else{
             $parentid = $staffid; // super admin is it's own parent

        }
        $array = array('parent' => $parentid[0]->parent_staff_id);
        $outputData = array_merge($data,$array);
        if (isset($data['id']) && !empty($data['id'])) {
            ...
            $this->db->insert('transactions', $outputData);
    ...
    public function get_monthTransaction($where,$staffid)
    {
        $result=$this->db->select('sum(amount) as total')->from('transactions');
        if($staffid != null)
        {

            $this->db->where("parent", $staffid);
