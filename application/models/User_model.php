...
class User_model extends MY_Model
{
    ...
    public function checkMpinLogin($data)
    {
        $this->db->select('id, username, password,role,is_active');
        $this->db->from('users');
        $this->db->where('mpin', $data['mpin']);
        $this->db->where('username', $data['username']);
        $this->db->limit(1);
        $query = $this->db->get();
        if ($query->num_rows() == 1) {
    ...
    public function isMpinExist($mpin)
    {
        $this->db->select('id');
        $this->db->from('users');
    ...
    {
        $this->db->where('id', $data['user_id']);
        $this->db->where('mpin', $data['current_pass']);
        $query = $this->db->get('users');
        if ($query->num_rows() > 0) {
            return true;
        } else {
    ...
    public function getUserByMobno($table, $role, $mobileno)
    {
        $this->db->select($table . '.*,users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`');
        $this->db->from($table);
        $this->db->join('users', 'users.user_id = ' . $table . '.id', 'left');
        $this->db->where('users.role', $role);
        if ($role == 'patient') {
            $this->db->where($table . '.mobileno', $mobileno);
        } 
        $query = $this->db->get();
        if ($mobileno != null) {
            return $query->row();
    ...
    public function forgotPasswordMobno($usertype, $mobileno)
    {
        $result = false;
       if ($usertype == 'patient') {
            $table  = "patients";
            $role   = "patient";
            $result = $this->getUserByMobno($table, $role, $mobileno);
        }
        return $result;
    }
    ...
    {
        $this->db->select('patients.*,users.user_id as `user_id` , users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`');
        $this->db->from('patients');
        $this->db->join('users', 'users.user_id = patients.id', 'left');
    ...
    public function checkmpinset($username){
        $this->db->select('patients.*,users.user_id as `user_id` , users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`,users.mpin as `user_mpin`');
        $this->db->from('patients');
        $this->db->join('users', 'users.user_id = patients.id', 'left');
        $this->db->where('users.role', 'patient');
        $this->db->where('users.username', $username);
