...
class Notificationsetting_model extends MY_Model
{
    ...
    //newly added
    public function getstaffidtosendSysNotification($id){ 
        return  $this->db->select('staff.id,staff.name,staff.surname,staff.employee_id,staff_roles.role_id')->from('staff')
        ->join("parent_child", "parent_child.parent_staff_id = staff.id", "left")
        ->join("staff_roles", "staff_roles.staff_id = staff.id", "left")
        ->where('parent_child.child_staff_id',$id)
        ->get()->result_array();

    }

    public function getstaffidtosendSysNotificationforPres($id){ 
        return  $this->db->select('staff.id,staff.name,staff.surname,staff.employee_id,staff_roles.role_id')->from('staff')
        ->join("parent_child", "parent_child.parent_staff_id = staff.id", "left")
        ->join("staff_roles", "staff_roles.staff_id = staff.id", "left")
        ->where('parent_child.child_staff_id',$id)
    ...
    public function getStafftonotifyPres($id){ 
        return  $this->db->select('staff.id,staff.name,staff.surname,staff.employee_id,staff_roles.role_id')->from('staff')
        ->join("parent_child", "parent_child.child_staff_id = staff.id", "left")
        ->join("staff_roles", "staff_roles.staff_id = staff.id", "left")
        ->join("roles", "roles.id = staff_roles.role_id", "left")
        ->where('parent_child.parent_staff_id',$id)
        ->or_where('staff.id',1)
        ->get()->result_array();
    }
    ...
    public function getCentralLogins(){ 
        return  $this->db->select('staff.id,staff.name,staff.surname,staff.employee_id,staff_roles.role_id')->from('staff')
        // ->join("parent_child", "parent_child.child_staff_id = staff.id", "left")
        ->join("staff_roles", "staff_roles.staff_id = staff.id", "left")
        // ->join("roles", "roles.id = staff_roles.role_id", "left")
        ->where('staff.created_by',7)

        ->where_in('staff_roles.role_id',[4,5,6])
        // ->or_where('staff.id',1)
        ->get()->result_array();
    }

    public function getStaffChild($id){
        return  $this->db->select('parent_child.*')->from('parent_child')
        ->where('parent_child.parent_staff_id',$id)
        ->get()->result_array();
    }
