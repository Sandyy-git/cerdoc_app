...
class Itemissue_model extends MY_Model
{
    ...
    {
        $added_by = $this->customlib->getLoggedInUserID();
        $this->datatables
            ...
            ->where('item_issue.added_by', $added_by)
            ->from('item_issue');
    ...
    //added one var on query $sql to show inventory
    public function issueinventoryreportRecord($start_date, $end_date) {
   
        $userLoggedIn = $this->customlib->getLoggedInUserId();

        if($userLoggedIn != 1){
        $sql="SELECT item_issue.*,item.name as `item_name`,item.item_category_id,item_category.item_category ,staff.employee_id,staff.name as dname ,staff.surname,roles.name FROM `item_issue` INNER JOIN item on item.id=item_issue.item_id INNER JOIN item_category on item_category.id=item.item_category_id INNER JOIN staff on staff.id=item_issue.issue_to INNER JOIN staff_roles on staff_roles.staff_id =staff.id INNER JOIN roles on roles.id= staff_roles.role_id where date_format(item_issue.issue_date,'%Y-%m-%d') >='". $start_date."'and date_format(item_issue.issue_date,'%Y-%m-%d') <= '".$end_date."' and  (item_issue.added_by) ='". $userLoggedIn ."'"; 
