...
class Pathology_category_model extends MY_model
{
    ...
    public function getCategoryName($id = null)
    {
        if (!empty($id)) {
            ...
            return $query->row_array();
             ...
             //na got gret cat baesed on luid
             $user_loginId = $this->customlib->getLoggedInUserID();

             if($user_loginId  != 1){
                $query = $this->db->where("added_by",$user_loginId)->get("pathology_category");
                }else{
    ...
    public function getpathoparameter($id = null)
    {
        ...
        if (!empty($id)) {
            ...
            if($user_loginId != 1){
            $this->db->where('pathology_parameter.added_by', $user_loginId);
            // $this->db->or_where('pathology_parameter.is_central_pharm', 'yes');
            }
            $query = $this->db->get();
            return $query->result_array();
        }
    ...
    public function getunit($id = null)
    {
        ...
        if (!empty($id)) {
            ...
            if($user_loginId  != 1){
            $this->db->where('added_by', $user_loginId);
            // $this->db->or_where('is_central_pharm', 'yes');
            $this->db->where('unit_type', 'patho');
