    ...
    public function getAllradiologytest()
    {
       ...
       if (!empty($custom_fields)) {
        ...
        $this->datatables
            ...
            if($user_loginId != 1){
            $this->datatables->where('radio.added_by', $user_loginId);
            // $this->datatables->or_where('radio.is_central_pharm', 'yes');
            }

            $this->datatables->from('radio');
        return $this->datatables->generate('json');
    } 

    public function getAllradiologybillRecord()
    {   
        //na radilioogy bill list
        ...
        //na e
         ...
         $this->datatables
             ...
             //na
             ->where_in('radiology_billing.generated_by', $coll_array)
 //na e
        ...
        $this->datatables
            ...
            ->sort('radiology_billing.id', 'desc')
            //na
            ->where('radiology_billing.generated_by', $login_user_id)
//na e
    ...
    public function getradiotestDetails()
    {
        ...
        $this->db->order_by('radio.id', 'desc');
          ...
          //na
          $this->db->where('radio.added_by', $staffRoles_id);
        //   $this->db->or_where('radio.is_central_pharm', 'yes');
          //ne
    ...
    public function getRadiology($id = null)
    {
        ...
        if( $userRoleIn[0]['created_by'] == 7 && $userRoleIn[0]['name'] == 'Radiologist'){
            // $userLoggedIn = $userRoleIn[0]['staff_id'];  //PREV
            $getfromcp = $this->role_model->getfromcpradio();
        ...
        if (!empty($id)) {
            ...
            if($userLoggedIn == 7){
                $this->db->where('radio.added_by',$userLoggedIn);
            }else{
                $this->db->where('radio.added_by',$userLoggedIn);
                // $this->db->or_where('radio.is_central_pharm','yes');
            }
   ...
   public function getDetails($id)
    ...
    public function addImport($medicine_data)
    {
        ...
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->insert('radiology_parameter', $medicine_data);
        $insert_id = $this->db->insert_id();
